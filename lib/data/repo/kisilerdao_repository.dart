...
class KisilerDaoRepository{
  ...
  List<Kisiler> parsKisiler(String cevap){
  ...
  Future<void> sil (int kisi_id) async{
    var url = "http://kasimadalan.pe.hu/kisiler/dekete_kisiler.php";
    var veri = {"kisi_id":kisi_id};
    var cevap = await Dio().post(url,data: FormData.fromMap(veri));
    print("Kisi Sil : ${cevap.data.toString()}");
  }

  Future<List<Kisiler>> kisisleriYukle () async{
    var url = "http://kasimadalan.pe.hu/kisiler/tum_kisiler.php";
    var cevap = await Dio().get(url);
    return parsKisiler(cevap.data.toString());
  }
